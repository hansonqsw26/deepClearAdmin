...
import {fetchClientQuotes} from "../../../../helper/quoteApi/quoteApi";

const InvoiceCard = () => {

    const [totalQuotesCount, setTotalQuotesCount] = useState(0);
    const [statusOneCount, setStatusOneCount] = useState(0);
    ...
    useEffect(() => {
        async function loadData() {
            setLoading(true);
            try {
                // Get clientId from localStorage, adjust if your storage is different
                const storedData = localStorage.getItem("adminUser");

                if (!storedData) throw new Error("Admin user not found");

                const {localId} = JSON.parse(storedData);

                if (!localId) throw new Error("Admin ID not found");

                const data = await fetchClientQuotes({localId, page: 1, limit: 1000}); // large limit to get all

                // data.data is assumed to be array of quotes (adjust if your API differs)
                const quotes = data.data || [];
                setTotalQuotesCount(quotes.length);

                // Count how many quotes have status === 1 (assuming quote object has 'status')
                // Note: if 'status' is nested or named differently, adjust accordingly
				const countStatusOne = quotes.filter(q => q.quote?.status === 1).length;
				setStatusOneCount(countStatusOne)
            } catch (error) {
            ...
            <div className="col-xl-3 col-xxl-6 col-sm-6">
                <div className="card bg-warning invoice-card">
                    <div className="card-body d-flex">
                        ...
                        <div>
                            <h2 className="text-white invoice-num">{totalQuotesCount}</h2>
                            <span className="text-white fs-18">Total Quote</span>
                        </div>
                    </div>
                </div>
            </div>
            <div className="col-xl-3 col-xxl-6 col-sm-6">
                <div className="card bg-success invoice-card">
                    <div className="card-body d-flex">
                        ...
                        <div>
                            <h2 className="text-white invoice-num">{statusOneCount}</h2>
                            <span className="text-white fs-18">Got Price</span>
                        </div>
                    </div>
                </div>
            </div>
